...
module caches.uim.caches.classes.cache copy;
 ...
 * ```
 * Cache.config("shared", [
 *   'className": UIM\Cache\ApcuEngine.classname,
 *   'prefix": 'my_app_'
 * ]);
 * ```
 *
...
class Cache {
    ...
    protected static CacheRegistry my_registry;

    // Returns the Cache Registry instance used for creating and using cache adapters.
    static CacheRegistry getRegistry() {
        return _registry.isNull ? new CacheRegistry() : _registry;
    }
    ...
    protected static void _buildEngine(string configName) {
        ...
        auto configData = configuration.data(configName);
        if (configData("className").isEmpty) {[]
    ...
    static bool write(string dataId, Json aValue, string configName = "default") {
        ...
        if (mysuccess == false && !myvalue.isEmpty) {
