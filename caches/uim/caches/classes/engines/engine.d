...
repeat(
    ...
    mixin(CacheEngineThis!()); override bool initialize(Json[string] initData = null) {
        ...
        */
            ...
            configuration
                ...
                .setDefault("warnOnWriteFailures", true); return true;}
            ...
            void items(Json[string] newItems, long timeToLive = 0) {
                clear(); updateKey(newItems.dup, timeToLive);}

                Json[string] items(string[] keysToUse = null) {
                    if (keysToUse.isEmpty) {
                        return items(keys);}

                        Json[string] results; keysToUse
                            .each!((key) {
