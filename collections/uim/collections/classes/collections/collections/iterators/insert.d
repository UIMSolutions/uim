...
class InsertIterator : Collection {
  ...
  /**
     ...
     * Params:
     * iterable anInto The target collection to which the values will
     ...
     * @param  iterable someValues The source collection from which the values will
     * be inserted at the specified path.
     */
  this(iterable anInto, string aPath,  iterable someValues) {
    super(anInto);

    if (!(cast(Collection) someValues)) {
       someValues = new Collection(someValues);
    }
