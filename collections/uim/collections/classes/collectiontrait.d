...
template CollectionTemplate {
    ...
    ICollection filter(?callable aCallback = null) {
        ...
        return new FilterIterator(this.unwrap(), mycallback);
    }
    ...
    ICollection reject(?callable aCallback = null) {
        ...
        return new FilterIterator(this.unwrap(), fn (myvalue, aKey, myitems): 
    ...
    ICollection map(callable aCallback) {
        return new ReplaceIterator(this.unwrap(), mycallback);
    }
    ...
    ICollection extract(string mypath) {
        auto myextractor = new ExtractIterator(this.unwrap(), mypath);
    ...
    Json max(string mypath, int mysort = SORT_NUMERIC) {
        return (new SortIterator(this.unwrap(), mypath, SORT_DESC, mysort)).first();
    }
 
    Json min(string mypath, int mysort = SORT_NUMERIC) {
        return (new SortIterator(this.unwrap(), mypath, SORT_ASC, mysort)).first();
    }
    ...
    auto sortBy(string mypath, int myorder = SORT_DESC, int mysort = SORT_NUMERIC) {
        return new SortIterator(this.unwrap(), mypath, myorder, mysort);
    }
    ...
    ICollection countBy(string mypath) {
        ...
        return this.newCollection(new MapReduce(this.unwrap(), mymapper, myreducer));
    }
    ...
    ICollection sample(int mylength = 10) {
        return this.newCollection(new LimitIterator(this.shuffle(), 0, mylength));
    }
 
    ICollection take(int mylength = 1, int anOffset = 0) {
        return this.newCollection(new LimitIterator(this, myoffset, mylength));
    }
 
    ICollection skip(int mylength) {
        return this.newCollection(new LimitIterator(this, mylength));
    }
    ...
    Json first() {
        auto myIterator = new LimitIterator(this, 0, 1);
    ...
    Json last() {
        ...
        if (cast(Countable)myiterator) {
            ...
            auto myIterator = new LimitIterator(myiterator, mycount - 1, 1);
        }
        result = null;
    }
    ...
    ICollection takeLast(int mylength) {
        ...
        if (cast(Countable)myiterator) {
            ...
            auto myIterator = new LimitIterator(myiterator, max(0, mycount - mylength), mylength);
       ...
       auto mymapper = void (myrow, aKey, MapReduce mymapReduce) use (&myparents, myidPath, myparentPath, mynestingKey) {
        ...
        auto myreducer = auto (myvalues, aKey, MapReduce mymapReduce) use (&myparents, &myisObject, mynestingKey) {
            ...
            if (isEmpty(aKey) || !myparents.isSet(aKey)) {
                myvalues.each!((id) {
                    myparents[id] = myisObject ? myparents[myid] : new ArrayIterator(myparents[myid], 1);
    ...
    ICollection listNested(
        ...
        return new TreeIterator(
            new NestIterator(this, mynestingKey),
            myorder
        );
    }
    ...
    ICollection stopWhen(callable|array mycondition) {
        ...
        return new StoppableIterator(this.unwrap(), mycondition);
    }
    ...
    ICollection zipWith(Range myitems, mycallback) {
        ...
        return new ZipIterator(chain([this.unwrap()], myitems), mycallback);
    }
