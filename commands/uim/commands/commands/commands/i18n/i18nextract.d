module uim.cake.commands.i18n.i18nextract;
...
class I18nExtractCommand : Command {
  mixin(CommandThis!("I18nExtractCommand"));
    ...
    protected void _getPaths(ConsoleIo aConsoleIo) {
  ...
  int execute(Arguments commandArguments, ConsoleIo aConsoleIo) {
        ...
        if (commandArguments.hasOption("extract-core")) {
           _extractCore = !((string)commandArguments.getOption("extract-core").toLower == "no");
        } else {
        ...
        if (commandArguments.hasOption("merge")) {
           _merge = !(strtolower((string)commandArguments.getOption("merge")) == "no");
        } else {
             aConsoleIo.writeln();
            response = aConsoleIo.askChoice(
    ...
    // Extract text
    protected void _extract(Arguments commandArguments, ConsoleIo aConsoleIo) {
         aConsoleIo.writeln();
         aConsoleIo.writeln();
     ...
     * Params:
     * \UIM\Console\Arguments commandArguments The io instance
     * @param \UIM\Console\ConsoleIo aConsoleIo The io instance
     */
     ...
     * Parse tokens
     * Params:
     * \UIM\Console\ConsoleIo aConsoleIo The io instance
     * @param string afunctionName auto name that indicates translatable string (e.g: '__")
     * @param array map Array containing what variables it will find (e.g: domain, singular, plural)
     */
    protected void _parse(ConsoleIo aConsoleIo, string afunctionName, array map) {
        count = 0;
     ...
     * Params:
     * \UIM\Console\Arguments commandArguments Console arguments
     */
     ...
     */
    protected void _writeFiles(Arguments commandArguments, ConsoleIo aConsoleIo) {
         aConsoleIo.writeln();
        overwriteAll = false;
     ...
     */
    protected void _markerError(ConsoleIo aConsoleIo, string afile, int lineNumber, string amarker, int count) {
