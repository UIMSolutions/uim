...
class DSqlserverDriver : DDriver {
    ...
    string dsn = "sqlsrv:Server={host}{port};Database={database};MultipleActiveResultsets=false"
        ...
        ~ (configuration.hasKey("connectionPooling") ? ";ConnectionPooling={connectionPooling}"
                : null)
        ~ (configuration.hasKey("failoverPartner") ? ";Failover_Partner={failoverPartner}" : null)
        ~ (configuration.hasKey(
                "loginTimeout") ? ";LoginTimeout={loginTimeout}" : null)
        ...
        ~ (
            configuration.hasKey("trustServerCertificate") ? ";TrustServerCertificate={trustServerCertificate}"
                : null);
...
protected ISelectQuery _selectQueryTranslator(SelectQuery aQuery) {
    ...
    return (currentVersion() < 11 && !anOffset.isNull)
        ? _pagingSubquery(aQuery, numberOfRows, anOffset) : _transformDistinct(aQuery);
}
     ...
     */
            protected void _transformFunctionExpression(FunctionExpression expression) {
                ...
                case "DATE_ADD":
                    ...
                    break;
                case "DAYOFWEEK" : expression
                        .name("DATEPART")
                        .conjunctionType(" ")
                        .add(["weekday, ": "literal"], [], true);
                    break;
                case "SUBSTR" : expression.name("SUBSTRING");
