...
*****************************************************************************************************************/
module uim.filesystems.classes.os.linux.filesystem;endTxt
...
class DLinuxFilesystem : DFilesystem {
  ...
  // #region isFolder
    override bool isFolder(string[] aPath, string aFolderName) {
      ...
      auto myPath = (aPath.length > 0 
        ? (aFolderName.length > 0 ? absolutePath(aPath~aFolderName) : absolutePath(aPath)) 
        : absolutePath(aFolderName));
    ...
    // #region Read
      // #region existsFolders() 
        ...
        override bool existsFolders(string[] aPath, string aFolderName) {
          ...
          bool result = false;
          () @trusted { result = std.file.dirEntries(myFolderPath, SpanMode.shallow).array.length > 0; } ();
          return result;
        }
      ...
      override IFolder[] folders(string[] aPath, string aFolderName) {
        ...
        IFolder[] results;
        () @trusted { 
          foreach (std.file.DirEntry entry;  
            std.file.dirEntries(myFolderPath, SpanMode.shallow)
      ...
      // #region countFolders() 
        ...
        override size_t countFolders(string[] aPath, string aFolderName) {
          ...
          // debug writeln("myFolderPath = ", myFolderPath);
          size_t result = false;
          () @trusted { result = std.file.dirEntries(myFolderPath, SpanMode.shallow).array.length; } ();
          return result;
        }
  ...
  override bool isFile(string[] aPath, string aFileName) {
    ...
    auto myPath = (aPath.length > 0 
      ? (aFileName.length > 0 ? absolutePath(aPath~aFileName) : absolutePath(aPath)) 
  ...
  override bool isLink(string[] aPath, string aLinkName) {
    ...
    auto myPath = (aPath.length > 0 
      ? (aLinkName.length > 0 ? absolutePath(aPath~aLinkName) : absolutePath(aPath)) 
      : absolutePath(aLinkName));
...
/*
  // #region LinkManagement
    ...
    override ILink createLink(string[] aPath) {
      if (!existsLink(aPath)) { return null; }
...
unittest {
  ...
  // #region Test Folder
    ...
    testFilesystem_CreateFolders(__MODULE__, myFileSystem, "LinuxFilesystem/folders/creatingFolders", ["LinuxFilesystem", "folders", "creatingFolders"]);
    
    myFileSystem.createIfNotExistsFolder("LinuxFilesystem/folders/readFolders");
    testFilesystem_ReadFolders(__MODULE__, myFileSystem, "LinuxFilesystem/folders/readFolders", ["LinuxFilesystem", "folders", "readFolders"]);
    
    myFileSystem.createIfNotExistsFolder("LinuxFilesystem/folders/updateFolders");
    testFilesystem_UpdateFolders(__MODULE__, myFileSystem, "LinuxFilesystem/folders/updateFolders", ["LinuxFilesystem", "folders", "updateFolders"]);
    
     if (!myFileSystem.existsFolder("LinuxFilesystem/folders/removeFolders")) { myFileSystem.createFolder("LinuxFilesystem/folders/removeFolders"); }
    testFilesystem_RemoveFolders(__MODULE__, myFileSystem, "LinuxFilesystem/folders/removeFolders", ["LinuxFilesystem", "folders", "removeFolders"]);  
  // #endregion Test Folder

  // #region Test Folder
    if (!myFileSystem.existsFolder("LinuxFilesystem/folder")) { myFileSystem.createFolder("LinuxFilesystem/folder"); }

    if (!myFileSystem.existsFolder("LinuxFilesystem/folder/creatingFolders")) { myFileSystem.createFolder("LinuxFilesystem/folder/creatingFolders"); }
    testFilesystem_CreateFolder(__MODULE__, myFileSystem, "LinuxFilesystem/folder/creatingFolders", ["LinuxFilesystem", "folder", "creatingFolders"]);
    
    if (!myFileSystem.existsFolder("LinuxFilesystem/folder/readFolders")) { myFileSystem.createFolder("LinuxFilesystem/folder/readFolders"); }
    testFilesystem_ReadFolder(__MODULE__, myFileSystem, "LinuxFilesystem/folder/readFolders", ["LinuxFilesystem", "folder", "readFolders"]);
    
    if (!myFileSystem.existsFolder("LinuxFilesystem/folder/updateFolders")) { myFileSystem.createFolder("LinuxFilesystem/folder/updateFolders"); }
    testFilesystem_UpdateFolder(__MODULE__, myFileSystem, "LinuxFilesystem/folder/updateFolders", ["LinuxFilesystem", "folder", "updateFolders"]);
    
     if (!myFileSystem.existsFolder("LinuxFilesystem/folder/removeFolders")) { myFileSystem.createFolder("LinuxFilesystem/folder/removeFolders"); }
    testFilesystem_RemoveFolder(__MODULE__, myFileSystem, "LinuxFilesystem/folder/removeFolders", ["LinuxFilesystem", "folder", "removeFolders"]);  
  // #endregion Test Folder

  // #region Test Files
    if (!myFileSystem.existsFolder("LinuxFilesystem/files")) { myFileSystem.createFolder("LinuxFilesystem/files"); }
    ...
    testFilesystem_CreateFiles(__MODULE__, myFileSystem, "LinuxFilesystem/files/creatingFiles", ["LinuxFilesystem", "files", "creatingFiles"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/files/readFiles")) { myFileSystem.createFolder("LinuxFilesystem/files/readFiles"); }
    testFilesystem_ReadFiles(__MODULE__, myFileSystem, "LinuxFilesystem/files/readFiles", ["LinuxFilesystem", "files", "readFiles"]);
    ...
    testFilesystem_UpdateFiles(__MODULE__, myFileSystem, "LinuxFilesystem/files/updateFiles", ["LinuxFilesystem", "files", "updateFiles"]);
    ...
    testFilesystem_RemoveFiles(__MODULE__, myFileSystem, "LinuxFilesystem/files/removeFiles", ["LinuxFilesystem", "files", "removeFiles"]);  
  // #endregion Test Files

  // #region Test File
    if (!myFileSystem.existsFolder("LinuxFilesystem/file")) { myFileSystem.createFolder("LinuxFilesystem/file"); }

    if (!myFileSystem.existsFolder("LinuxFilesystem/files/creatingFiles")) { myFileSystem.createFolder("LinuxFilesystem/files/creatingFiles"); }
    testFilesystem_CreateFile(__MODULE__, myFileSystem, "LinuxFilesystem/files/creatingFiles", ["LinuxFilesystem", "files", "creatingFiles"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/files/readFiles")) { myFileSystem.createFolder("LinuxFilesystem/files/readFiles"); }
    testFilesystem_ReadFile(__MODULE__, myFileSystem, "LinuxFilesystem/files/readFiles", ["LinuxFilesystem", "files", "readFiles"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/files/updateFiles")) { myFileSystem.createFolder("LinuxFilesystem/files/updateFiles"); }
    testFilesystem_UpdateFile(__MODULE__, myFileSystem, "LinuxFilesystem/files/updateFiles", ["LinuxFilesystem", "files", "updateFiles"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/files/removeFiles")) { myFileSystem.createFolder("LinuxFilesystem/files/removeFiles"); }
    testFilesystem_removeFile(__MODULE__, myFileSystem, "LinuxFilesystem/files/removeFiles", ["LinuxFilesystem", "files", "removeFiles"]);  
  // #endregion Test File

  // #region Test Links
    if (!myFileSystem.existsFolder("LinuxFilesystem/links")) { myFileSystem.createFolder("LinuxFilesystem/links"); }
    
    if (!myFileSystem.existsFolder("LinuxFilesystem/links/creatingLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/creatingLinks"); }
    testFilesystem_CreateLinks(__MODULE__, myFileSystem, "LinuxFilesystem/links/creatingLinks", ["LinuxFilesystem", "links", "creatingLinks"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/links/readLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/readLinks"); }
    testFilesystem_ReadLinks(__MODULE__, myFileSystem, "LinuxFilesystem/links/readLinks", ["LinuxFilesystem", "links", "readLinks"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/links/updateLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/updateLinks"); }
    testFilesystem_UpdateLinks(__MODULE__, myFileSystem, "LinuxFilesystem/links/updateLinks", ["LinuxFilesystem", "links", "updateLinks"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/links/removeLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/removeLinks"); }
    testFilesystem_RemoveLinks(__MODULE__, myFileSystem, "LinuxFilesystem/links/removeLinks", ["LinuxFilesystem", "links", "removeLinks"]); 
  // #endregion Test Links
  
    // #region Test Link
    if (!myFileSystem.existsFolder("LinuxFilesystem/link")) { myFileSystem.createFolder("LinuxFilesystem/link"); }
    
    if (!myFileSystem.existsFolder("LinuxFilesystem/links/creatingLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/creatingLinks"); }
    testFilesystem_CreateLink(__MODULE__, myFileSystem, "LinuxFilesystem/links/creatingLinks", ["LinuxFilesystem", "links", "creatingLinks"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/links/readLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/readLinks"); }
    testFilesystem_ReadLink(__MODULE__, myFileSystem, "LinuxFilesystem/links/readLinks", ["LinuxFilesystem", "links", "readLinks"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/links/updateLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/updateLinks"); }
    testFilesystem_UpdateLink(__MODULE__, myFileSystem, "LinuxFilesystem/links/updateLinks", ["LinuxFilesystem", "links", "updateLinks"]);

    if (!myFileSystem.existsFolder("LinuxFilesystem/links/removeLinks")) { myFileSystem.createFolder("LinuxFilesystem/links/removeLinks"); }
    testFilesystem_RemoveLink(__MODULE__, myFileSystem, "LinuxFilesystem/links/removeLinks", ["LinuxFilesystem", "links", "removeLinks"]); 
  // #endregion Test Link
