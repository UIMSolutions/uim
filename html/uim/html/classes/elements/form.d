...
class DH5Form : DH5Obj {
	...
	// TODO mixin(TypeField!("string", "acceptCharset", "AcceptCharset"));

	/* action - Specifies where to send the form-data when a form is submitted */
	// TODO mixin(TypeField!("string", "action", "Action"));

	/* autoComplete - Specifies whether a form should have autocomplete on or off */
	// TODO mixin(TypeField!("string", "autoComplete", "Autocomplete"));
	...
	void add(Methods value) {
		_attributes.removeByKey("method");
	...
	void add(FormTargets value) {
		_attributes.removeByKey("target");
...
string toString(FormTargets value) {
...
string toString(EncTypes value) {
	return cast(string) value;
}
