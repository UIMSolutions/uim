...
class DSession {
    ...
    this(Json[string] configData = null) {
        ...
        if (!configData.isEmpty("handler")) {
            ...
            configuration.removeByKey("handler.engine");
    ...
    Json consume(string key) {
        ...
        if (!result.isNull) {
            ...
            _overwrite(_SESSION, Hash.removeByKey(_SESSION, key));
        }
        return result;
    }
    ...
    bool removeByKey(string sessionName) {
        if (this.check(sessionName)) {
            /** @psalm-suppress InvalidScalarArgument */
            _overwrite(_SESSION, Hash.removeByKey(_SESSION, sessionName));
        }
    }
...
/*    protected void _overwrite(Json[string] oldValues, Json[string] newValues) {
            ...
            oldValues.byKeyValue
                ...
                .each!(kv => removeByKey(oldValues[kv.key]));
