...
class DMoI18NParser : DI18NParser {
    ...
    Json[string] parse(string fileName) {
        if (!fileName.exists) {
            throw new UIMException("`%s` not found".format(fileName));
        }

        if (!fileName.isFile) {
            throw new UIMException("`%s` not a file".format(fileName));
        }

        auto fileStream = File(fileName, "rb");
        if (fileStream.error) {
            throw new UIMException("Cannot open resource `%s`".format(fileName));
        }
