...
class DFileJsonCollection : DJsonCollection {
  ...
  override Json[] findMany(bool allVersions = false) {
    ...
    // Body
    auto ids = folder.folders
      .map!(f => folder.name)
      .filter!(id => id.isUUID)
      .array; // get all valid ids
  ...
  override Json findOne(STRINGAA select, bool allVersions = false) {
    ...
    return ("versionNumber" !in select
        ? findOne(UUID(select["id"]), allVersions) : findOne(UUID(select["id"]), to!size_t(
  ...
  override Json insertOne(Json newData) {
    ...
    // Body
    if ("id" !in newData)
      newData["id"] = randomUUID.toString;
    if ("versionNumber" !in newData)
      newData["versionNumber"] = 1;
  ...
  override bool removeOne(Json select, bool allVersions = false) {
    ...
    auto versionFile = idFolder.file("versionNumber" in select ? select["versionNumber"].get!string
        : "1");
    if (versionFile is null) {
      return false;
    }
