...
bool hasDataFormat(string dataFormatName) {
  return dataFormats.any!(dataFormat => dataFormat == dataFormatName);
}

void addDataFormats(string[] newDataFormats) {
  newDataFormats.each!(dataFormat => addDataFormat(dataFormat));
}

void addDataFormat(string newDataFormat) {
  if (!hasDataFormat(newDataFormat)) {
      _dataFormats ~= df;
  }
}
