...
class DAttribute { // TODO:  /* DEntity,  */ IAttribute {
  ...
  /* 
  auto Attribute(UUID id) {
    return new DAttribute(id);
  } */ 

  auto Attribute(string newName) {
    return new DAttribute(newName);
  }

  /* 
  auto Attribute(UUID id, string name) {
    return new DAttribute(id, name);
  }

  auto Attribute(Json json) {
    return new DAttribute(json);
  } */

  auto Attribute(Json[string] initData) {
    return new DAttribute(initData);
  }

  auto Attribute(string newName, Json[string] initData) {
    return new DAttribute(newName, initData);
  }
