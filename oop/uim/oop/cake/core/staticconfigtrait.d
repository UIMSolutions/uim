...
template StaticConfigTemplate {
    ...
    static Json getConfigOrFail(string aKey) {
        if (!isSet(configuration.data(aKey])) {
            throw new InvalidArgumentException("Expected configuration `%s` not found.".format(aKey));
        }
...
REGEXP;
        ...
        if (!parsed) {
            throw new InvalidArgumentException("The DSN string `%s` could not be parsed.".format(dsn));
        }
        exists = [];d
        /**
