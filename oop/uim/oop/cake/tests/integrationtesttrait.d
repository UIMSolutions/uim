...
template IntegrationTestTemplate {
     ...
     */
    protected string[] _unlockedFields = [];
    ...
    void controllerSpy(IEvent event, ?Controller controller = null) {
        if (!controller) {
            ...
            assert(cast(Controller)controller);
        }
    ...
    protected void _handleError(Throwable exceptionToHandle) {
        ...
        if (isEmpty(className) || !class_exists(className)) {
    ...
    protected array _buildRequest(string aurl, string amethod, string[] adata = []) {
        ...
        if (isSet(_request["headers"])) {
            foreach (_request["headers"] as myKey: v) {
                name = strtoupper(myKey.replace("-", "_"));
    ...
    protected array _addTokens(string aurl, array data) {
        if (_securityToken == true) {
            ...
            auto formProtector = new FormProtector(["unlockedFields": _unlockedFields]);
        ...
        if (_csrfToken == true) {
            auto middleware = new CsrfProtectionMiddleware();
    ...
    void assertResponseError(string failureMessage = null) {
        this.assertThat(null, new StatusError(_response), failureMessage);
    }
    
    // Asserts that the response status code is in the 5xx range.
    void assertResponseFailure(string failureMessage= null) {
        this.assertThat(null, new StatusFailure(_response), failureMessage);
    }
    ...
    void assertResponseCode(int statusCode, string failureMessage = null) {
        this.assertThat(statusCode, new StatusCode(_response), failureMessage);
    }
    
    /**
     * Asserts that the Location header is correct. Comparison is made against a full URL.
     * Params:
     * string[]|null url The URL you expected the client to go to. This
     ...
     */
    void assertRedirect(string[]|null url = null, string failureMessage= null)) {
        if (!_response) {
     ...
     * @param string amessage The failure message that will be appended to the generated message.
     */
    void assertRedirectEquals(string[]|null url = null, string failureMessage = null) {
        if (!_response) {
            this.fail("No response set, cannot assert header.");
        }
    ...
    void assertSession(Json expected, string aPath, string message = "") {
        verboseMessage = this.extractVerboseMessage(message);
        this.assertThat(expected, new SessionEquals(somePath), verboseMessage);
    }
    ...
    void assertSessionHasKey(string aPath, string amessage = "") {
        verboseMessage = this.extractVerboseMessage(message);
        this.assertThat(somePath, new SessionHasKey(somePath), verboseMessage);
    }
    ...
    void assertSessionNotHasKey(string aPath, string amessage = null) {
        verboseMessage = this.extractVerboseMessage(message);
        this.assertThat(somePath, this.logicalNot(new SessionHasKey(somePath)), verboseMessage);
    }
    ...
    void assertFlashMessage(string aexpected, string aKey = "flash", string amessage = null) {
        verboseMessage = this.extractVerboseMessage(message);
        this.assertThat(expected, new FlashParamEquals(_requestSession, aKey, "message"), verboseMessage);
    }
    ...
    void assertFlashMessageAt(int at, string aexpected, string aKey = "flash", string amessage = "") {
        ...
        this.assertThat(
            expected,
            new FlashParamEquals(_requestSession, aKey, "message", at),
            verboseMessage
        );
    }
    ...
    void assertFlashElement(string aexpected, string aKey = "flash", string amessage = "") {
        ...
        this.assertThat(
            expected,
            new FlashParamEquals(_requestSession, aKey, "element"),
            verboseMessage
        );
    }
    ...
    void assertFlashElementAt(int at, string aexpected, string aKey = "flash", string amessage = "") {
        ...
        this.assertThat(
            expected,
            new FlashParamEquals(_requestSession, aKey, "element", at),
            verboseMessage
        );
    }
