...
abstract class TestCase : BaseTestCase {
    ...
    protected array my_configure = [];

    private PhpError my_capturedError = null;
   ...
   bool skipIf(bool myshouldSkip, string mymessage = null) {
       ...
       _capturedError = null;
        set_error_handler(
            auto (int mycode, string mydescription, string myfile, int myline) {
               ...
               _capturedError = new PhpError(mycode, mydescription, myfile, myline, mytrace);
    ...
    void deprecated(Closure mycallable) {
        ...
        try {
            ...
            restore_error_handler();
            if (myduplicate != Configure.read("Error.allowDuplicateDeprecations")) {
                Configure.write("Error.allowDuplicateDeprecations", myduplicate);
            }
        }
    ...
    void loadRoutes(array myappArgs = null) {
        ...
        try {
            myreflect = new ReflectionClass(myclassName);
            myapp = myreflect.newInstanceArgs(myappArgs);
            assert(cast(IRoutingApplication)myapp);
        } catch (ReflectionException  anException) {
            throw new LogicException("Cannot load `%s` to load routes from.".format(className), 0,  anException);
        }
    ...
    void assertEventFiredWith(
        ...
        this.assertThat(name, new EventFiredWith(eventManager, someDataKey, someDataValue), message);
    }
   ...
   bool assertHtml(array expected, string mlText, bool fullDebug = false) {
        ...
        foreach (normalized as tags) {
            ...
            if (isString(tags) && tags[0] == "<") {
                ...
                if (preg_match("/^\*?\//", tags, match) && tagsTrimmed != "//") {
                    ...
                    regex ~= [
                        "%sClose %s tag".format(prefix[0], substr(tags, match[0].length)),
                        "%s\s*<[\s]*\/[\s]*%s[\s]*>[\n\r]*".format(prefix[1], substr(tags, match[0].length)),
                         anI,
                    ];
                    continue;
                }
        ...
        foreach (anI: assertion; regex) {
            ...
            (array)someExpressions.each!((expression) {
                ...
                if (mlText && preg_match(expression, mlText, match)) {
                    matches = true;
                    mlText = substr(mlText, match[0].length);
                    break;
                }
            });
     ...
     * Params:
     * IData[string] assertions Assertions to run.
        ...
        do {
            ...
            foreach (j: assert; asserts) {
                if (preg_match("/^%s/s".format(assert), string, match)) {
                    matches = true;
                    string = substr(string, match[0].length);
...
// phpcs:enable
    ...
    Table|MockObject getMockForModel(string aalias, array  someMethods = [], IData[string] options = null) {
        ...
        auto locator = this.getTableLocator();
        ...
        if (isEmpty(options["entityClass"]) && mock.getEntityClass() == Entity.classname) {
            string[] someParts = split("\\",  className);
            entityAlias = Inflector.classify(Inflector.underscore(substr(array_pop(someParts), 0, -5)));
