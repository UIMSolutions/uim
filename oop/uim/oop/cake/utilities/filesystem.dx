...
class Filesystem {
    ...
    Iterator find(string mypath, Closure|string myfilter = null, size_t flags = 0) {
        ...
        auto mydirectory = new FilesystemIterator(mypath, myflags);
    ...
    Iterator findRecursive(string mypath, Closure|string myfilter = null, int myflags = null) {
        myflags ??= FilesystemIterator.KEY_AS_PATHNAME
            | FilesystemIterator.CURRENT_AS_FILEINFO
            | FilesystemIterator.SKIP_DOTS;
        mydirectory = new RecursiveDirectoryIterator(mypath, myflags);
        mydirFilter = new RecursiveCallbackFilterIterator(
            mydirectory,
        ...
        myflatten = new RecursiveIteratorIterator(
            mydirFilter,
    ...
    protected Iterator filterIterator(Iterator myiterator, Closure|string myfilter) {
        if (isString(myfilter)) {
            return new RegexIterator(myiterator, myfilter);
        }
        return new CallbackFilterIterator(myiterator, myfilter);
    }
   ...
   bool copyDir(string mysource, string destinationPath) {
        auto destPath = (new SplFileInfo(destinationPath)).getPathname();
        ...
        bool result = true;
        new FilesystemIterator(mysource).each!((fileInfo) {
            if (fileInfo.isDir()) {
