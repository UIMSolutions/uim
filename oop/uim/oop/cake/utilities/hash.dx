...
class Hash {
    ...
    static Json get(array data, string[] mypath, Json mydefault = null) {
        if (mydata.isEmpty || mypath.isNull) {
            return mydefault;
        }
        if (isString(mypath) || isInt(mypath)) {
            string[] myparts = to!string(mypath.split("."));
        } else {
            myparts = mypath;
        }
        switch (count(myparts)) {
            ...
            default:
                myparts.each!((key) {
                    if ((isArray(mydata) || cast(ArrayAccess)mydata) && isSet(mydata[aKey])) {
                        mydata = mydata[aKey];
                    } else {
                        return mydefault;
                    }
    ...
    static ArrayAccess|array extract(ArrayAccess|array data, string mypath) {
        ...
        if (!preg_match("/[{\[]/", mypath)) {
            mydata = get(mydata, mypath);
            if (mydata !isNull && !(isArray(mydata) || cast(ArrayAccess)mydata)) {
                return [mydata];
            }
        ...
        mytokens.each!((token) {
            ...
            if (myconditions) {
                myfilter = [];
                mynext
                    .filter!(item => (isArray(item) || cast(ArrayAccess)item) && _matches(item, myconditions))
    ...
    protected static array _splitConditions(string mytoken) {
        ...
        if (myposition == -1) {
            myconditions = substr(mytoken, myposition);
            mytoken = substr(mytoken, 0, myposition);
        }
    ...
    protected static bool _matches(ArrayAccess|array data, string myselector) {
        ...
        conditions.each!((condition) {
            ...
            myprop = mydata[myattr] ?? "";
            myisBool = isBool(myprop);
     ...
     */
    static array insert(array data, string mypath, Json myvalues = null) {
     ...
     */
    protected static array _simpleOp(string myop, array data, array mypath, Json myvalues = null) {
        my_list = &mydata;
    ...
    static array remove(array data, string mypath) {
        ...
        if (mynoExpansion && mynoTokens && !mypath.has(".")) {
            unset(mydata[mypath]);
    ...
    static array combine(
        array data,
        string[]|null mykeyPath,
        string[]|null myvaluePath = null,
        string mygroupPath = null
    ) {
        if (isEmpty(mydata)) {
            return null;
        }
        if (isArray(mykeyPath)) {
     ...
     */
    static array merge(array data, Json mymerge) {
     ...
     * @param callable myfunction The auto to call on each extracted value.
     */
    static Json reduce(array data, string mypath, callable myfunction) {
        myvalues = (array)extract(mydata, mypath);
     ...
     * @param callable myfunction The auto to call on each extracted value.
     */
    static Json apply(array data, string mypath, callable myfunction) {
        myvalues = (array)extract(mydata, mypath);
    ...
    static array sort(
        ...
        foreach (someKeys as myKey) {
            ...
            if (isSet(myoriginalKeys[myKey])) {
                mysorted[myoriginalKeys[myKey]] = mydata[myoriginalKeys[myKey]];
            } else {
    ...
    static array diff(array data, array mycompare) {
        ...
        myintersection = array_intersect_key(mydata, mycompare);
        while ((aKey = key(myintersection)) !isNull) {
            if (mydata[aKey] == mycompare[aKey]) {
                unset(mydata[aKey], mycompare[aKey]);
            }
            next(myintersection);
        }
    ...
    static array mergeDiff(array data, array mycompare) {
        ...
        foreach (mycompare as aKey: myvalue) {
            if (!array_key_exists(aKey, mydata)) {
                mydata[aKey] = myvalue;
            } else if (isArray(myvalue) && isArray(mydata[aKey])) {
     ...
     */
    static array normalize(array data, bool myassoc = true, Json mydefault = null) {
    ...
    static array<array> nest(array data, IData[string] optionData = null) {
        ...
        foreach (mydata as result) {
            ...
            myidMap[myid] = isSet(myidMap[myid][options["children"]])
                ? array_merge(result, myidMap[myid])
                : array_merge(result, [options["children"]: []]);

            if (!myparentId || !in_array(myparentId, myids)) {
                result ~= &myidMap[myid];
            } else {
                myidMap[myparentId][options["children"]] ~= &myidMap[myid];
            }
        }
        if (!result) {
