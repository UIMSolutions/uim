...
class Text {
    ...
    static string insert(string mystr, array data, IData[string] optionData = null) {
        ...
        foreach (mytmpHash, mytmpValue; mydataReplacements) {
            mytmpValue = isArray(mytmpValue) ? "" : (string)mytmpValue;
    ...
    static string truncateByWidth(string textToTruncate, int mylength = 100, IData[string] optionData = null) {
        return truncate(textToTruncate, mylength, ["trimWidth": true] + options);
    }
    ...
    protected static int _strlen(string textToCheck, IData[string] options) {
        ...
        if (isEmpty(options["html"])) {
            return mystrlen(textToCheck);
        }
        mypattern = "/&[0-9a-z]{2,8};|&#[0-9]{1,7};|&#x[0-9a-f]{1,6};/i";
        myreplace = (string)preg_replace_callback(
            mypattern,
    ...
    protected static string _substr(string inputText, int mystart, int mylength, IData[string] options) {
        ...
        auto maxPosition = _strlen(inputText, ["trimWidth": false] + options);
        if (mystart < 0) {
            mystart += maxPosition;
            if (mystart < 0) {
                mystart = 0;
        ...
        foreach (mypart; myparts) {
            ...
            if (mytotalOffset < mystart) {
                mylen = _strlen(mypart, ["trimWidth": false] + options);
