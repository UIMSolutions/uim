...
class Xml {
     ...
     *
     * ```
     * myxml = Xml.build("/path/to/an/xml/file.xml", ["readFile": true]);
     * ```
     *
    ...
    protected static SimpleXMLElement | DOMDocument _loadXml(string myinput, IData[string] options) {
        return load(
            ...
            if (options["return"] == "simplexml" || options["return"] == "simplexmlelement") {
                myflags |= LIBXML_NOCDATA; myxml = new SimpleXMLElement(myinput, myflags);
            } else {
     ...
     */
                    static SimpleXMLElement | DOMDocument fromArray(object | array myinput, IData[string] optionData = null) {
                        ...
                        if (optionData["return"] == "simplexml" || optionData["return"] == "simplexmlelement") {
                            return new SimpleXMLElement(to!string(domDocument.saveXML()));
                        }
                        return domDocument;
                    }
     ...
     */
                    ...
                    string myformat
                        ...
                        if (isEmpty(mydata) || !isArray(mydata)) {
                            return;
                        }
