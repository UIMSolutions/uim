...
class Hash {
    ...
    static IData get(array data, string[] mypath, IData mydefault = null) {
        ...
        switch (count(myparts)) {
            ...
            default:
                myparts.each!((key) {
                    if ((isArray(mydata) || cast(DArrayAccess)mydata) && isSet(mydata[aKey])) {
                        mydata = mydata[aKey];
                    } else {
                        return mydefault;
                    }
    ...
    static ArrayAccess|array extract(ArrayAccess|array data, string mypath) {
        ...
        if (!preg_match("/[{\[]/", mypath)) {
            ...
            if (mydata !isNull && !(isArray(mydata) || cast(DArrayAccess)mydata)) {
                return [mydata];
            }
     ...
     *
     * ```
     * Hash.sort(mydata, "some.attribute", "asc", ["type": "regular", "ignoreCase": BooleanData(true)]);
     * ```
     *
