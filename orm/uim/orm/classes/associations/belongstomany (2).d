 ...
 */
class DBelongsToMany : DAssociation {
    ...
    protected ITable _junctionTable;
     ...
     * @var string[]|string
     */
    protected string[] _targetForeignKey = null;
    ...
    Table junction(Table|string aTable = null) {
        ...
        if (source.aliasName() == target.aliasName()) {
            throw new DInvalidArgumentException(
                ...
                .format(this.name, source.aliasName())
            );
        }
        ...
        if (!junction.hasAssociation(tAlias)) {
            ...
            if (
                ...
                throw new DInvalidArgumentException(
                    ...
                    "is incompatible with the `{this.name}` association on `{source.aliasName()}`"
                );
            }
        }
        if (!junction.hasAssociation(sAlias)) {
    ...
    void attachTo(SelectQuery aQuery, IData[string] options = null) {
        ...
        assoc = this.getTarget().getAssociation(junction.aliasName());
        ...
        aQuery.getEagerLoader().addToJoinsMap(junction.aliasName(), assoc, true);
    ...
    protected void _appendNotMatching(SelectQuery aQuery, IData[string] options = null) {
        ...
        aQuery
            .andWhere(function (QueryExpression exp) use (subquery, conds) {
                auto anIdentifiers = conds.keys.map!(field => new DIdentifierExpression(field)).array;
    ...
    protected bool _saveLinks(IEntity sourceEntity, array targetEntities, IData[string] options = null) {
        ...
        string[] assocForeignKey = (array)belongsTo.getForeignKeys();
        ...
        return this.junction().getConnection().transactional(
            auto () use (sourceEntity, targetEntities, primaryValue, options) {
                ...
                foreach (aKey; array_merge(assocForeignKey, junctionPrimaryKey)) {
                    ...
                    matchesConditions[aliased] = new DIdentifierExpression(junctionQueryAlias ~ "." ~ aKey);
                }
