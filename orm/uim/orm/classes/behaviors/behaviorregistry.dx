...
class DBehaviorRegistry : ObjectRegistry, IEventDispatcher {
    ...
    // Method mappings.
    protected array[string] my_methodMap = null;

    // Finder method mappings.
    protected array[string] my_finderMap = null;
     ...
     */
    protected IBehavior _create(object|string myclass, string aliasToObject, IData[string] configData) {
        if (isObject(myclass)) {
            return myclass;
        }
   ...
   bool hasFinder(string mymethod) {
     ...
     */
    IData call(string mymethod, array myargs = []) {
        mymethod = mymethod.toLower;
