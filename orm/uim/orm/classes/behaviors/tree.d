...
class DTreeBehavior : DBehavior {
    ...
    override bool initialize(Json[string] myConfiguration) {
        ...
        configuration
            ...
            .setDefault("parent", "parent_id")
            .setDefault("left", "lft")
            .setDefault("right", "rght")
            .setDefault("scope", Json(null))
            .setDefault("level", Json(null))
            .setDefault("recoverOrder", Json(null))
            .setDefault("cascadeCallbacks", false);

        // TODO configuration.set("leftField", new DIdentifierExpression(configuration.get("left")));
        // TODO configuration.set("rightField", new DIdentifierExpression(configuration.get("right")));
    ...
    protected void _setChildrenLevel(IORMEntity ormEntity) {
        ...
        auto primaryKeysValue = entity.get(primaryKeys);
        auto depths = [primaryKeysValue: entity.get(configuration.get("level"))];

        auto children = _table.find("children", [
                "for": primaryKeysValue,
                "fields": [
                    primaryKeys(), configuration.get("parent"),
     ...
     */
        protected void _setAsRoot(IORMEntity ormEntity) {
            ...
            entity
                .set(configuration.get("left"), edge - diff)
                .set(configuration.get("right"), edge);
        }
     ...
     */
        protected void _unmarkInternalTree() {
            ...
            _table.updateAll(
                ...
                /** @var DDBExpression\QueryExpression exp */
                leftInverse = exp.clone; leftInverse.conjunctionType("*").add("-1");
                    rightInverse = leftInverse.clone; return exp
                    .eq(configuration.get("leftField"), leftInverse.add(configuration.get("leftField")))
                    .eq(configuration.get("rightField"), rightInverse.add(
                        configuration.get("rightField")));},
                    function(exp) use(myConfiguration) {
                        /** @var DDBExpression\QueryExpression exp */
     ...
     */
                DORMQuery findPath(DORMQuery query, Json[string] options = null) {
                    ...
                    [left, right] = array_map(
                        function(field) { return _table.aliasField(field); },
                        [configuration.get("left"), configuration.get("right")]
                    );

                    node = _table.get(options.get("for"), [
                            "fields": [left, right]
                        ]);
                ...
                int countChildNodes(IORMEntity entityNode, bool shouldCountDirect = false) {
                    ...
                    return (entityNode.get(
                        configData.get("right")) - entityNode.get(
                        configData.get("left")) - 1) / 2;
                }
     ...
     */
                DORMQuery findChildren(DORMQuery query, Json[string] options = null) {
                    ...
                    auto updatedOptions = options.setPath(
                        ["for": Json(null), "direct": false.toJson]);
                    [
                        parent, left, right
                    ] = array_map(
                        function(field) { return _table.aliasField(field); },
     ...
     */
                DORMQuery formatTreeList(DORMQuery query, Json[string] options = null) {
                    return query.formatResults(
                        ...
                        nested = results.listNested(); return nested.printer(
                        options.get("valuePath"), options.get("keyPath"), options.get(
                        "spacer"));});
                    }
            ...
            return _removeFromTree(node);
                    ...
                    protected IORMEntity _removeFromTree(
                        ...
                        auto left = nodeToRemove.get(
                            configuration.get("left"));
                        auto right = nodeToRemove.get(
                            configuration.get("right"));
                        auto parent = nodeToRemove.get(
                            configuration.get("parent"));
                        nodeToRemove.set(
                            configuration.get("parent"), null);
                        ...
                        _table.updateAll(
                            ...
                            [
                                configuration.get(
                                "parent"): nodeToRemove.get(primary)
                            ]
                        );
                        _sync(1, "-", "BETWEEN " ~ (
                            left + 1) ~ " AND " ~ (right - 1));
                        _sync(2, "-", "> {right}");
                        edge = _getMax();
                        nodeToRemove.set(configuration.get(
                            "left"), edge + 1);
                        nodeToRemove.set(
                            configuration.get("right"), edge + 2);
                        auto fields = [
                            configuration.get("parent"),
                            configuration.get("left"),
                            configuration.get("right")
                        ];
                        _table.updateAll(nodeToRemove.extract(fields), [
                                primary: nodeToRemove.get(primary)
                            ]);
                        fields.each!(field => nodeToRemove.setDirty(
                            field, false));
                        return nodeToRemove;
                    }
     ...
     */
                    IORMEntity moveUp(IORMEntity nodeToMove, size_t placesToMove = 1) {
                        ...
                        return _table.getConnection()
                            .transactional(function() use(
                                node, placesToMove) {
                                _ensureFields(nodeToMove); return _moveUp(nodeToMove, numplacesToMoveber);
                            });
                    }
                    ...
                    protected IORMEntity _moveUp(IORMEntity nodeToMove, size_t placesToMove) {
                        ...
                        [
                            ...
                            nodeLeft,
                            nodeRight
                        ] = node.extract([parent, left,right]).values;
                    ...
                    return exp.lt(configuration.get("rightField"), nodeLeft);
                        ...
                        [targetLeft] = array_values(targetNode.extract(
                            [left, right]));
                        edge = _getMax();
            ...
            return _moveDown(node, number);
                    ...
                    protected IORMEntity _moveDown(IORMEntity node, int number) {
                        auto configData = configuration
                            .data;
                        [parent, left, right] = [configuration
                            .get("parent"], configuration.get(
                                "left"), configuration.get(
                                "right")]; [
                                    nodeParent,
                                    nodeLeft,
                                    nodeRight
                                ] = array_values(node.extract([
                                    parent,
                                    left,
                                    right
                                ])); auto targetNode = null; if (number != true) {
                                /** @var DORMdatasources.IORMEntity|null targetNode */
                                targetNode = _scope(
                                    _table.find())
                                    .select([left, right])
                                    ...
                                    .where(function(exp) use(
                                        ...
                                        if (!targetNode) {
                                            ...
                                            myConfiguration, nodeRight) {
                                                ...
                                                [, targetRight] = targetNode.extract([left, right])
                                                .values; auto edge = _getMax(); auto leftBoundary = nodeRight + 1;
                                                auto rightBoundary = targetRight; auto nodeToEdge = edge - nodeLeft + 1;
                                                ...
                                                nodeRight - nodeLeft)); node.set(right, targetRight);

                                                node.setDirty(left, false); node.setDirty(right, false);
                                                        ...
                                                        void recover() {
                                                            ...
                                                            void() {
                                                                ...
                                                                protected int _getMax() {
                                                                    ...
                                                                    "right"); auto rightField = configuration.get(
                                                                    "rightField");
                                                                    auto edge = _scope(
                                                                    _table.find())
                                                                    .select([
                                                                    ...
                                                                    .first(); return edge == null || edge.isEmpty(
                                                                    field)
                                                                    ? 0 : edge[field];
                                                                }
     ...
     */
                                                                protected void _sync(int shiftvalue, string dir, string conditions, bool shouldMark = false) {
                                                                    auto configData = configuration
                                                                    .data; foreach ([configuration.get(
                                                                        ...
                                                                        auto movement = exp
                                                                        .clone; movement.add(field)
                                                                        .add( /* (string) */ shiftvalue)
                                                                        .conjunctionType(
                                                                        dir); auto inverse = exp
                                                                        .clone; movement = shouldMark ?
     ...
     */
                                                                    ...
                                                                    int getLevel(entity) {
                                                                        ...
                                                                        auto configData = configuration
                                                                        .data; auto entity = _table.find("all")
                                                                        .select([
                                                                            configuration.get("left"),
                                                                            configuration.get(
                                                                            "right")
