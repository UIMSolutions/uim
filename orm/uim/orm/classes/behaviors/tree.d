...
class DTreeBehavior : DBehavior {
    ...
    protected void _unmarkInternalTree() {
        ...
        /* function(exp) use(myConfiguration) {
            /** @var DDBExpression\QueryExpression exp * /
            leftInverse = exp.clone; leftInverse.conjunctionType("*").add("-1"); rightInverse = leftInverse
                .clone; return exp
                ...
                .eq(configuration.get("rightField"), rightInverse.add(
                    configuration.get("rightField")));},/* function(exp) use(myConfiguration) {
                    /** @var DDBExpression\QueryExpression exp * /
    ...
    protected IORMEntity _moveUp(IORMEntity nodeToMove, size_t placesToMove) {
        ...
        if (!targetNode) {
            ...
            targetNode = _scope(
                _table.find())
                .select([left, right])
                .where(["parent IS": nodeParent]) /* .where(function(exp) use(myConfiguration, nodeLeft) {
                    /** @var DDBExpression\QueryExpression exp * /
                    return exp.lt(configuration.get("rightField"), nodeLeft);
                    }) */
        ...
        /* [targetLeft] = array_values(targetNode.extract(
                    [left, right])); */
        auto edge = _getMax();
        auto leftBoundary = targetLeft;
        auto rightBoundary = nodeLeft - 1;

        auto nodeToEdge = edge - nodeLeft + 1;
        auto shift = nodeRight - nodeLeft + 1;
        auto nodeToHole = edge - leftBoundary + 1;
