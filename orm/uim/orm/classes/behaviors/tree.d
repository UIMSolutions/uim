...
class DTreeBehavior : DBehavior {
     ...
     */
                DORMQuery findPath(DORMQuery query, Json[string] options = null) {
                    ...
                    int countChildNodes(IORMEntity entityNode, bool shouldCountDirect = false) {
                        auto configData = configuration.data; auto parent = _table.aliasField(
                            configData.get("parent")); if (shouldCountDirect) {
                            return _scope(_table.find())
                                .where([parent: entityNode.get(primaryKeys())])
                                .count();}

                            _ensureFields(entityNode); return (entityNode.get(
     ...
     */
                            DORMQuery findChildren(DORMQuery query, Json[string] options = null) {
                                auto configData = configuration.data; auto updatedOptions = options.setPath(
                                ...
                                if (query.clause("order") == null) {
                                    ...
                                    node = _getNode(for); return _scope(query)
                                        .where([
                                            "{right} <": node.get(configuration.get("right")),
                                            "{left} >": node.get(configuration.get("left")),
                                        ]);}
     ...
     */
                                    DORMQuery findTreeList(DORMQuery query, Json[string] options = null) {
                                        auto left = _table.aliasField(
                                            this.configuration.get("left")); auto results = _scope(
                                            query)
                                            .find("threaded", [
     ...
     */
            ...
            return _removeFromTree(node);
                                                ...
                                                IORMEntity nodeToRemove) {
                                                    ...
                                                    configuration.get("left")); auto right = nodeToRemove.get(
                                                    configuration.get("right")); auto parent = nodeToRemove.get(
                                                    configuration.get("parent")); nodeToRemove.set(
                                                    ...
                                                    primary = primaryKeys(); _table.updateAll(
                                                    [
                                                    ...
                                                    left + 1) ~ " AND " ~ (right - 1));
                                                    _sync(2, "-", "> {right}"); edge = _getMax();
                                                    nodeToRemove.set(configuration.get(
                                                    "left"), edge + 1); nodeToRemove.set(
                                                    ...
                                                    field, false)); return nodeToRemove;
                                                }
     ...
     */
            ...
            return _moveDown(node, number);
                                                                    ...
                                                                    protected IORMEntity _moveDown(IORMEntity node, int number) {
                                                                        ...
                                                                        .data; [parent, left, right] = [configuration
                                                                            ...
                                                                            [, targetRight] = array_values(
                                                                            targetNode.extract([left, right]));
                                                                            edge = _getMax();
     ...
     */
                                                                            ...
                                                                            IORMEntity ormEntity) {
                                                                                ...
                                                                                auto values = filterValues(entity.extract(fields));
                                                                                if (count(values) == count(
                                                                                    fields)) {
                                                                                    return;
                                                                                }

                                                                                fresh = _table.get(entity.get(
                                                                                primaryKeys()));
