...
class Marshaller
{
     ...
     */
    protected array _buildPropertyMap(array data, IData[string] optionData) {
        map = null;
        ...
        foreach (include as key: nested) {
            ...
            if (isset(options["isMerge"])) {
                callback = function (value, entity) use (assoc, nested) {
                    ...
                    options = nested + ["associated": ArrayData, "association": assoc];

                    return _mergeAssociation(entity.get(assoc.getProperty()), assoc, value, options);
                };
            } else {
                callback = function (value, entity) use (assoc, nested) {
                    options = nested + ["associated": ArrayData];
     ...
     * @param bool isNew Whether it is a new DORMEntity or one to be updated.
     * @return array The list of validation errors.
     * @throws \RuntimeException If no validator can be created.
     */
    protected function _validate(array data, IData[string] optionData, bool isNew) {
        if (!options["validate"]) {
            return [];
        }
        ...
        } elseif (options["validate"].isString) {
     ...
     */
    protected function _prepareDataAndOptions(array data, IData[string] optionData) {
     ...
     */
    protected function _marshalAssociation(Association assoc, value, IData[string] optionData) {
        if (!(value.isArray) {
            return null;
        }
     ...
     */
    function many(array data, IData[string] optionData = null) {
        output = null;
     ...
     */
    protected function _belongsToMany(BelongsToMany assoc, array data, IData[string] optionData = null) {
    ...
    protected function _loadAssociatedByIds(Association assoc, array ids) {
        ...
        if (multi) {
            ...
            filter = new DTupleComparison(primaryKeys, ids, type, "IN");
        } else {
     ...
     * @see DORMEntity::_accessible
     */
    function merge(IEntity anEntity, array data, IData[string] optionData = null): IEntity
    {
        [data, options] = _prepareDataAndOptions(data, options);
     ...
     */
    function mergeMany(Range entities, array data, IData[string] optionData = null) {
        primary = (array)_table.getPrimaryKeys();

        indexed = (new DCollection(data))
        ...
        conditions = (new DCollection(indexed))
        ...
        foreach ((new DCollection(indexed)).append(new) as value) {
            if (!(value.isArray) {
                continue;
            }
     ...
     * @return DORMDatasource\IEntity|array<DORMDatasource\IEntity>|null
     */
    protected function _mergeAssociation(original, Association assoc, value, IData[string] optionData) {
        if (!original) {
            return _marshalAssociation(assoc, value, options);
        }
        if (!(value.isArray) {
     ...
     * @return array<DORMDatasource\IEntity>
     */
    protected function _mergeBelongsToMany(array original, BelongsToMany assoc, array value, IData[string] optionData) {
        associated = options["associated"]  [];
     ...
     * @return array<DORMDatasource\IEntity> An array of entities
     */
    protected function _mergeJoinData(array original, BelongsToMany assoc, array value, IData[string] optionData) {
     ...
     */
    protected void dispatchAfterMarshal(IEntity anEntity, array data, IData[string] optionData = null) {
        data = new ArrayObject(data);
        options = new ArrayObject(options);
