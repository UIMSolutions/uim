 ...
 * ```
 *
 * Counter cache using lambda auto returning the count
 * This is equivalent to example #2
 *
 * ```
 * [
 *    "Users": [
 *        "posts_published": auto (IEvent myevent, IEntity myentity, Table mytable) {
 *            myquery = mytable.find("all").where([
 *                "published": true,
 *                "user_id": myentity.get("user_id")
 *            ]);
 ...
 * ```
 *
 * When using a lambda auto you can return `false` to disable updating the counter value
 ...
 *
 * ```
 * this.Articles.save(myarticle, ["ignoreCounterCache": true]);
 * ```
 */
class CounterCacheBehavior : Behavior {
     ...
     * @param \ArrayObject<string, mixed> options The options for the query
     */
    void afterDelete(IEvent myevent, IEntity myentity, ArrayObject options) {
        if (isSet(options["ignoreCounterCache"]) && options["ignoreCounterCache"] == true) {
            return;
        }
       _processAssociations(myevent, myentity);
    ...
    protected void _processAssociation(
        ...
        myforeignKeys = (array)myassoc.getForeignKey();
