...
class EagerLoader {
     ...
     */
    protected IData[string] my_containments = [];
     ...
     */
    protected EagerLoadable|array|null my_normalized = null;
    ...
    void setMatching(string myassociationPath, ?Closure mybuilder = null, IData[string] optionData = null) {
       _matching ??= new static();
        ...
        mysharedOptions = ["negateMatch": false, "matching": true] + options;
        ...
        mynested = ["matching": true, "queryBuilder": mybuilder] + options;
     ...
     */
    protected EagerLoadable _normalizeContain(Table myparent, string myalias, IData[string] options, array mypaths) {
        ...
        myeagerLoadable = new EagerLoadable(myalias, configData);
            ...
            myaliases
                ...
                .each!((configData) {
                    foreach (configData as myloadable) {
                        assert(cast(EagerLoadable)myloadable);
     ...
     */
    protected EagerLoadable[] _resolveJoins(array myassociations, array mymatching = []) {
        auto result;
     ...
     */
    protected EagerLoadable[] _buildAssociationsMap(array mymap, array mylevel, bool mymatching = false) {
    ...
    protected array _collectKeys(array myexternal, SelectQuery myquery, array results) {
        ...
        foreach (myexternal as mymeta) {
            myinstance = mymeta.instance();
            if (!myinstance.requiresKeys(mymeta.getConfig())) {
                continue;
            }
