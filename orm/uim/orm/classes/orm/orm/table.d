 ...
 * - `afterMarshal(IEvent myevent, IEntity myentity, ArrayObject options)`
 * - `buildValidator(IEvent myevent, Validator myvalidator, string myname)`
 * - `buildRules(RulesChecker myrules)`
 * - `beforeRules(IEvent myevent, IEntity myentity, ArrayObject options, string myoperation)`
 * - `afterRules(IEvent myevent, IEntity myentity, ArrayObject options, bool result, string myoperation)`
...
class Table : IRepository, IEventListener, IEventDispatcher, IValidatorAware {
    ...
    protected string my_table = null;
     ...
     * Connection instance
     */
    protected Connection my_connection = null;

    // The schema object containing a description of this table fields
    protected TableISchema my_schema = null;
    ...
    protected AssociationCollection my_associations;

    // BehaviorRegistry for this table
    protected BehaviorRegistry my_behaviors;
     ...
     * Params:
     * IConfigData[string] configData List of options for this table.
     */
     ...
     *
     * ```
     * auto initialize(IConfigData[string] configData = null)
     * {
     ...
     * ```
     * Params:
     * IConfigData[string] configData Configuration options passed to the constructor
     */
    ...
    Connection getConnection() {
        if (!_connection) {
            ...
            assert(cast(Connection)myconnection);
           _connection = myconnection;
        }
        return _connection;
    }
    ...
    string[]|string getPrimaryKey() {
        if (_primaryKey.isNull) {
            aKey = this.getSchema().getPrimaryKey();
            if (count(aKey) == 1) {
                aKey = aKey[0];
            }
           _primaryKey = aKey;
    ...
    string[] getDisplayField() {
        ...
        return _displayField = this.getPrimaryKey();
    }
     ...
     * @throws \UIM\ORM\Exception\MissingEntityException when the entity class cannot be found
     */
   ...
   bool hasAssociation(string myname) {
     ...
     * string myname The alias used for the association.
     */
    protected Association findAssociation(string myname) {
        if (!myname.has(".")) {
    ...
    protected IEntity|array _processFindOrCreate(
        ...
        if (result == false) {
            throw new PersistenceFailedException(myentity, ["findOrCreate"]);
        }
        return myentity;
    }
     ...
     */
    protected SelectQuery _getFindOrCreateQuery(SelectQuery|callable|array mysearch) {
     ...
     * myarticles.save(myentity, ["associated": false]);
     * ```
     * Params:
    ...
    IEntity saveOrFail(IEntity myentity, IData[string] optionData = null) {
        ...
        if (mysaved == false) {
            throw new PersistenceFailedException(myentity, ["save"]);
        }
        return mysaved;
    }
    ...
    protected IEntity|false _processSave(IEntity myentity, ArrayObject options) {
        ...
        if (!mysuccess && myisNew) {
            myentity.unset(this.getPrimaryKey());
    ...
    protected IEntity _insert(IEntity myentity, array data) {
        auto primaryKey = (array)this.getPrimaryKey();
        ...
        if (mystatement.rowCount() != 0) {
            mysuccess = myentity;
            myentity.set(myfilteredKeys, ["guard": false]);
            myschema = this.getSchema();
     ...
     * Params:
     * string[] myprimary The primary key columns to get a new ID for.
    ...
    protected auto _saveMany(
        ...
        if (myfailed !isNull) {
            ...
            throw new PersistenceFailedException(myfailed, ["saveMany"]);
        }
        mycleanupOnSuccess = void (IEntity myentity) use (&mycleanupOnSuccess) {
            myentity.clean();
            myentity.setNew(false);
            ...
            foreach (myentity.toArray().keys as myfield) {
                ...
                } else if (isArray(myvalue) && cast(IEntity)current(myvalue)) {
    ...
    iterable<\UIM\Datasource\IEntity> deleteManyOrFail(iterable myentities, IData[string] optionData = null) {
        ...
        if (myfailed !isNull) {
            throw new PersistenceFailedException(myfailed, ["deleteMany"]);
        }
        return myentities;
    }
     ...
     * Params:
     * \UIM\Datasource\IEntity myentity The entity to delete.
    ...
    protected bool _processDelete(IEntity myentity, ArrayObject options) {
        ...
        myprimaryKey = (array)this.getPrimaryKey();
   ...
   bool hasFinder(string mytype) {
    ...
    SelectQuery<TSubject> invokeFinder(Closure mycallable, SelectQuery myquery, array myargs) {
        myreflected = new ReflectionFunction(mycallable);
     ...
     */
    protected SelectQuery _dynamicFinder(string mymethod, array myargs) {
   ...
   bool __isSet(string associationName) {
     ...
     */
    bool validateUnique(Json aValue, IData[string] options, array mycontext = null) {
        if (mycontext.isNull) {
            mycontext = options;
        }
        myentity = new Entity(
