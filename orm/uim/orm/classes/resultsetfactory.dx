...
class ResultSetFactory {
    ...
    protected array collectData(SelectQuery myquery) {
        ...
        mydata["matchingAssoc"] = (new Collection(myassocMap))
            .match(["matching": true])
            .indexBy("alias")
            .toArray();

        mydata["containAssoc"] = (new Collection(array_reverse(myassocMap)))
    ...
    protected IEntity|array groupResult(array myrow, IData[string] metadata) {
        ...
        foreach (myassoc; mydata["containAssoc"]) {
            ...
            assert(cast(Association)myinstance);
