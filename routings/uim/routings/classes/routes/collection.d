...
class DRouteCollection {
     ...
     */
    string match(Json[string] url, Json[string] mycontext) {
        ...
        throw new DMissingRouteException(["url": var_export_(url, true), "context": mycontext]);
    }

    protected string matchRoute(Route routeToCheck, Json[string] url, Json[string] mycontext) {
        if (auto match = routeToCheck.match(url, mycontext)) {
