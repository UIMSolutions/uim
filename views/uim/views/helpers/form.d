 ...
 *
 * @method string text(string aFieldName, IData[string] options  = null) Creates input of type text.
 * @method string number(string aFieldName, IData[string] options  = null) Creates input of type number.
 * @method string email(string aFieldName, IData[string] options  = null) Creates input of type email.
 * @method string password(string aFieldName, IData[string] options  = null) Creates input of type password.
 * @method string search(string aFieldName, IData[string] options  = null) Creates input of type search.
...
class FormHelper : Helper {
     ...
     */
    protected WidgetLocator my_locator;
     ...
     * Context factory.
     */
    protected ContextFactory my_contextFactory = null;
     ...
     */
    protected FormProtector myformProtector = null;
    ...
    this(View myview, IData[string] configData = null) {
        ...
        if (isSet(configData["locator"])) {
            mylocator = configData["locator"];
            unset(configData["locator"]);
        }
        if (isSet(configData["widgets"])) {
            if (isString(configData["widgets"])) {
                configData["widgets"] = (array)configData["widgets"];
            }
            mywidgets = configData["widgets"] + mywidgets;
            unset(configData["widgets"]);
        }
        if (isSet(configData["groupedInputTypes"])) {
           _groupedInputTypes = configData["groupedInputTypes"];
            unset(configData["groupedInputTypes"]);
        }
     ...
     */
    ContextFactory contextFactory(?ContextFactory myinstance = null, array mycontexts = []) {
        if (myinstance.isNull) {
            return _contextFactory ??= ContextFactory.createWithDefaults(mycontexts);
        }
     ...
     */
    string create(Json formContext = null, IData[string] options  = null) {
        myappend = "";
       ...
       _lastAction(myurl);
        ...
        myactionAttr = mytemplater.formatAttributes(["action": myaction, "escape": false]);
     ...
     * Params:
     * string[]|null myurl The URL of the last form.
     */
    protected void _lastAction(string[]|null myurl = null) {   myaction = Router.url(myurl, true);
     ...
     * Params:
     * string views The dot separated name for the field.
     */
    void unlockField(string views) {
        this.getFormProtector().unlockField(views);
    }
     ...
     */
    protected FormProtector createFormProtector(array myformTokenData) {
     ...
     * @param string[]|null mytext Error message as string or array of messages. If an array,
     *  it should be a hash of key names: messages.
     * @param IData[string] options See above.
     */
    string error(string myfield, string[]|null mytext = null, IData[string] options  = null) {
     ...
     * @param IData[string] options Options array. Valid keys are:
     *
     ...
     */
    protected Json _extractOption(string views, IData[string] options, Json mydefault = null) {
    ...
    protected string _inputLabel(string fieldName, string labelText = null, string[string] labelAttributes = null, IData[string] labelOptions = null) {
        IData[string] options += ["id": Json(null), "input": Json(null), "nestedInput": Json(false), "templateVars": Json.emptyArray];
     ...
     * @param IData[string] options Array of HTML attributes.
     */
    string postLink(string mytitle, string[]|null myurl = null, IData[string] options  = null) {
     ...
     */
    string widget(string views, array data = []) {
        mysecure = null;
        if (isSet(mydata["secure"])) {
            mysecure = mydata["secure"];
            unset(mydata["secure"]);
        }
        mywidget = _locator.get(views);
     ...
     */
    void resetTemplates() {
        this.setTemplates(_defaultConfigData["templates"]);
    }
...
>>>>>>> a8eca63e3a082caffb32183a18c571cd53fc1ac0
    ...
    Json getSourceValue(string fieldName, IData[string] options  = null) {
        auto myvalueMap = [
            "data": "getData",
            "query": "getQuery",
        ];
