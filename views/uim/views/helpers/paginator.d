...
class PaginatorHelper : Helper {
     ...
     */
    void setPaginated(IPaginated mypaginated, IData[string] options  = null) {
    ...
    array params() {
        return _configData.isSet("params") + this.paginated().pagingParams();
    }
     ...
     */
    protected string _toggledLink(string|false mytext, bool myenabled, IData[string] options, IData[string] mytemplates) {
        ...
        mytext = options["escape"] ? h(mytext): mytext;
     ...
     * @param IData[string] options Options for pagination link. See above for list of keys.
     */
    string prev(string mytitle = "<< Previous", IData[string] options  = null) {
        mydefaults = [
            "url": [],
     ...
     * @param IData[string] options Options for pagination link. See above for list of keys.
     */
    string next(string mytitle = "Next >>", IData[string] options  = null) {
        mydefaults = [
            "url": [],
            "disabledTitle": mytitle,
            "escape": true,
     ...
     * @param IData[string]|string|null mytitle Title for the link. If mytitle.isNull aKey will be used
     ...
     * @param IData[string] options Options for sorting link. See above for list of keys.
     */
    string sort(string aKey, string[]|null mytitle = null, IData[string] options  = null) {
        options += ["url": [], "escape": true];
        myurl = options["url"];
        unset(options.remove("url");
        ...
        mysortKey = (string)this.param("sort");
        ...
        if (isArray(mytitle) && array_key_exists(mydir, mytitle)) {
            mytitle = mytitle[mydir];
        }
        mypaging = ["sort": aKey, "direction": mydir, "page": 1];

        myvars = [
            "text": options["escape"] ? h(mytitle): mytitle,
     ...
     * Params:
     * IData[string] options Pagination options.
     * @param array myurl URL.
     * @param IData[string] myurlOptions Array of options
     */
    string generateUrl(
     ...
     * Merges passed URL options with current pagination state to generate a pagination URL.
     * Params:
     * IData[string] options Pagination/URL options array
     * @param array myurl URL.
     */
    array generateUrlParams(IData[string] options  = null, array myurl = []) {
        ...
        if (!empty(_config["options"]["routePlaceholders"])) {
            myplaceholders = array_flip(_config["options"]["routePlaceholders"]);
     ...
     * Params:
     * IData[string] options Options for the numbers.
     */
    string numbers(IData[string] options  = null) {
        mydefaults = [
     ...
     * @param IData[string] options Options from the numbers() method.
     */
    protected string _formatNumber(StringTemplate mytemplater, IData[string] options) {
        myvars = [
            "text": options["text"],
     ...
     * @param IData[string] options Options from the numbers() method.
     */
    protected string _modulusNumbers(StringTemplate mytemplater, array myparams, IData[string] options) {
        string result = "";
     ...
     * @param IData[string] options Options from the numbers() method.
         */
    protected string _firstNumber(string myellipsis, array myparams, int mystart, IData[string] options) {
        string result = "";
     ...
     * @param IData[string] options Options from the numbers() method.
     */
    protected string _lastNumber(string myellipsis, array myparams, int myend, IData[string] options) {
        string result = "";
     ...
     * @param IData[string] options Options from the numbers() method.
     */
    protected string _numbers(StringTemplate mytemplater, array myparams, IData[string] options) {
        string result = "";
        result ~= options["before"];
     ...
     */
    string first(string|int myfirst = "<< first", IData[string] options  = null) {
        options += [
            "url": [],
            "escape": true,
        ];
        ...
        if (isInt(myfirst) && this.paginated().currentPage() >= myfirst) {
            ...
            myfirst = options["escape"] ? h(myfirst): myfirst;
     ...
     */
    string last(string|int mylast = "last >>", IData[string] options  = null) {
        options += [
            "escape": true,
            "url": [],
        ];
        ...
        if (isInt(mylast) && mycurrentPage <= mylower) {
            ...
            mylast = options["escape"] ? h(mylast): mylast;
     ...
     */
    string meta(IData[string] options  = null) {
        options += [
            "block": false,
            "prev": true,
            "next": true,
     ...
     * @param IData[string] options Options for Select tag attributes like class, id or event
     */
    string limitControl(array mylimits = [], int mydefault = null, IData[string] options  = null) {
        mylimits = mylimits ?: [
            "20": "20",
            "50": "50",
            "100": "100",
